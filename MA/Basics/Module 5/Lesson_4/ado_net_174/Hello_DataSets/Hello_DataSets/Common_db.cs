   ...
   public class Common_db
    {
        ...
        private string MyConnectionString;// = @"Data Source=.\SQLEXPRESS;Initial Catalog=ShopDB; Integrated Security=True";               //строка подключения
        // implement Common_db constructor with string parameter for connection string
        internal Common_db(string myConnectionString)
        {
        ...
        public bool MyConnect()
        {
            ...
            // даже если в блоке using генерируется исключительная ситуация
            {
                try
                {
                    ...
                    result = true;                                    // assign true for result
                }
                catch (Exception e)
                {
                    Console.WriteLine();
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine(e.Message);                     // exception message output
